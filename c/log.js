module.exports = new class extends Controller {
	...
	_login(data,ip,stime) {
		this.redis.hget("user", data.openid, user => {
			...
			this.mysql.one("user", {
				...
				if ( !dbuser ) {
					this.redis.hdel("user", data.openid);
					return this._one();	
				}
				else this.mysql.update("user", {
					oauth : dbuser.oauth,
					last_time : stime,
					last_time_day : day()
				}, dbuser.id, over => {
					user.id = dbuser.id;
					this.redis.hset("userid", dbuser.id, data.openid);
					this.redis.hset("user", data.openid, user);
					this.mysql.insert("userlogin", {
						user_id : dbuser.id,
						login_day : day(),
						login_time : stime
					}, over => {
		...
		this.redis.hget("user", data.openid, user => {
			...
			this.mysql.one("user", {
				...
				if ( dbuser ) {
					...
					this.redis.hset("userid", user.id, data.openid);
					this.redis.hset("user", data.openid, user);
					this.mysql.insert("userlogin", {
						user_id : user.id,
						login_day : day(),
	...
	//用户玩解锁游戏记录
	_addAndUpUserPlayGame(data,ip,stime){
	...
	//用户对游戏打分和评论
	_addUserGameScore(data,ip,stime){
	...
	//订阅时增加订阅记录
	_addUserSubscribe(data,ip,stime){
		this.mysql.one("user_subscribe", {
			open_id : data.open_id,
			pro_id : data.pro_id
		}, resuser_subscribe => {
			if( !resuser_subscribe ) this.mysql.insert("user_subscribe", {
				open_id : data.open_id,
				pro_id : data.pro_id,
				sub_time : stime
			}, over => {
	...
	//取消订阅时删除订阅记录
	_deleteUserSubscribe(data,ip,stime){
	...
	//增加用户登录日时间戳、时间戳
	_add_tg_userontime(data,ip,stime){
	...
	//修改用户登录时间戳
	_update_tg_userontime(data,ip,stime){
	...
	//结束用户登录时长生命周期（昨天）
	_over_tg_userontime(data,ip,stime){
		this.mysql.update("tg_userontime",{
	...
	//结束用户登录时长生命周期(当天)
	_over2_tg_userontime(data,ip,stime){
		this.mysql.update("tg_userontime",{
			exittime : data.exittime,
			"ontime +" : data.ontime
		},{
	...
	//用户开始玩游戏（计时开始）
	_add_tg_gameontime(data,ip,stime){
	...
	//结束用户玩游戏（计时结束）
	_over_tg_gameontime(data,ip,stime){
	...
	//重新连接找到上一条数据
	_reconnect_tg_userontime(data,ip,stime){
		this.mysql.update("tg_gameontime",{
	...
	_addModelformid(data, ip, stime){
		if( typeof data.openid == "undefined" || !data.openid ) this._one();
		else this.mysql.insert("userformid", {
			openid : data.openid,
			formid : data.formid,
			addtime : stime
		},over => {
	...
	//增加用户分享次数
	_addUserShare(data, ip, stime){
		if ( typeof data.game_id == "undefined" || data.game_id == "" ) data.game_id = 0;
		this.mysql.one("usershare", {
			user_id : data.user_id,
			share_day : data.share_day,
			game_id : data.game_id
		}, resusershare => {
			if( !resusershare ) this.mysql.insert("usershare", {
				user_id : data.user_id,
				share_day : data.share_day,
				game_id : data.game_id
			}, over => {
			...
			else this.mysql.update("usershare", {
				"share_times +" : 1
			}, {
				user_id : data.user_id,
				share_day : data.share_day,
	...
	//修改数据库中project表订阅人数
	_updateProject(data,ip,stime){
		this.mysql.update("project", {
